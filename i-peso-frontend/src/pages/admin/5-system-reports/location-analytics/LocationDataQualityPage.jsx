...
export default function LocationDataQualityPage() {
  ...
  const MetricCard = ({ title, data, icon: Icon, colorClass }) => {
    if (!data) return null
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <MetricCard 
          title="Job Seekers" 
          data={metrics?.job_seekers} 
          icon={Users} 
          colorClass="bg-blue-50 text-blue-600" 
        />
        <MetricCard 
          title="Employers" 
          data={metrics?.employers} 
          icon={Building2} 
          colorClass="bg-indigo-50 text-indigo-600" 
        />
        <MetricCard 
          title="Job Vacancies" 
          data={metrics?.job_vacancies} 
          ...
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={analytics?.seekers_by_city || []} layout="vertical" margin={{ left: 40 }}>
                ...
                <RechartsTooltip />
                <Bar dataKey="count" fill="#3b82f6" radius={[0, 4, 4, 0]} name="Seekers" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={analytics?.vacancies_by_city || []} layout="vertical" margin={{ left: 40 }}>
                ...
                <YAxis type="category" dataKey="city" width={100} tick={{ fontSize: 12 }} />
                <RechartsTooltip />
                <Bar dataKey="count" fill="#10b981" radius={[0, 4, 4, 0]} name="Vacancies" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
