...
const initialFilters = {
  ...
  date_to: today.toISOString().slice(0, 10), period: 'monthly', city: '',
}
...
function AnalyticsView({ data, loading, error, filters, appliedFilters, setFilters, options, chips, applyFilters, resetFilters, removeFilter }) {
  ...
  return (
    <>
      <Card padding="none" className="overflow-hidden transition-all duration-300 hover:shadow-lg hover:shadow-brand-500/5">
        <form onSubmit={applyFilters} className="bg-gradient-to-r from-slate-50 to-white p-5 sm:p-6">
          <div className="mb-4 flex items-center justify-between gap-3">
            <div><h2 className="font-bold text-slate-950">Macro Analytics Filters</h2><p className="text-sm text-slate-500">All data and generated reports follow this applied range.</p></div>
            <CalendarRange className="h-5 w-5 text-brand-700" />
          </div>
          <div className="flex flex-wrap items-end gap-4">
            <div className="flex-1 min-w-[140px]"><Filter label="Date from"><input type="date" max={filters.date_to || undefined} value={filters.date_from} onChange={(e) => setFilters({ ...filters, date_from: e.target.value })} /></Filter></div>
            <div className="flex-1 min-w-[140px]"><Filter label="Date to"><input type="date" min={filters.date_from || undefined} value={filters.date_to} onChange={(e) => setFilters({ ...filters, date_to: e.target.value })} /></Filter></div>
            <div className="flex-1 min-w-[140px]"><SelectFilter label="Period" value={filters.period} onChange={(value) => setFilters({ ...filters, period: value })} options={[['monthly', 'Monthly'], ['yearly', 'Yearly']]} /></div>
            <div className="flex-1 min-w-[140px]"><SelectFilter label="City / Municipality" value={filters.city} onChange={(value) => setFilters({ ...filters, city: value })} options={toOptions(options.cities)} /></div>
            <div className="flex items-end gap-2 shrink-0"><Button type="submit" disabled={loading}>{loading ? 'Applying…' : 'Apply filters'}</Button><Button type="button" variant="outline" onClick={resetFilters} disabled={loading} aria-label="Reset filters"><RotateCcw className="h-4 w-4" /></Button></div>
          </div>
          <div className="mt-5 flex flex-wrap items-center gap-2" aria-label="Applied analytics filters">
            ...
            {chips.map(([key, value]) => <span key={key} className="inline-flex max-w-full items-center gap-1 rounded-full bg-slate-100 py-1 pl-3 pr-1 text-xs font-semibold text-slate-700 transition hover:bg-slate-200"><span className="truncate">{pretty(key)}: {value}</span><button type="button" onClick={() => removeFilter(key)} className="rounded-full p-1 hover:bg-slate-300" aria-label={`Remove ${pretty(key)} filter`}><X className="h-3 w-3" /></button></span>)}
          </div>
        </form>
      </Card>
...
function ChartCard({ title, subtitle, children }) { return <Card className="min-w-0 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:shadow-brand-500/10"><div className="mb-5"><h3 className="font-bold leading-5 text-slate-950">{title}</h3>{subtitle && <p className="mt-1 text-xs leading-5 text-slate-500">{subtitle}</p>}</div>{children}</Card> }
function TrendChart({ series, lines }) { return series.length ? <div className="h-72 min-w-0" role="img" aria-label={`Line chart of ${lines.join(' and ')} across ${series.length} periods.`}><ResponsiveContainer><LineChart data={series} margin={{ top: 8, right: 12, left: -12, bottom: 4 }}><CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" /><XAxis dataKey="period" tickFormatter={formatPeriod} tick={{ fontSize: 10, fill: '#64748b' }} minTickGap={24} /><YAxis allowDecimals={false} tick={{ fontSize: 10, fill: '#64748b' }} /><Tooltip content={<ChartTooltip />} cursor={{ stroke: '#94a3b8', strokeWidth: 1, strokeDasharray: '4 4' }} /><Legend verticalAlign="bottom" height={30} wrapperStyle={{ fontSize: 11, paddingTop: 8, cursor: 'pointer' }} />{lines.map((line, index) => <Line key={line} type="monotone" dataKey={line} stroke={COLORS[index]} strokeWidth={3} dot={{ r: 2.5 }} activeDot={{ r: 7, strokeWidth: 0, fill: COLORS[index] }} animationDuration={1000} />)}</LineChart></ResponsiveContainer></div> : <Empty /> }
function Donut({ data = [] }) { return data.length ? <div className="space-y-3"><div className="h-48" role="img" aria-label={`Proportional chart. ${seriesSummary(data)}.`}><ResponsiveContainer><PieChart><Pie data={data} dataKey="value" nameKey="label" innerRadius={48} outerRadius={76} paddingAngle={2} animationDuration={800}>{data.map((item, index) => <Cell key={item.label} fill={COLORS[index % COLORS.length]} className="hover:opacity-80 transition-opacity outline-none" />)}</Pie><Tooltip content={<ChartTooltip />} /></PieChart></ResponsiveContainer></div><div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2">{data.map((item, index) => <div key={item.label} className="flex min-w-0 items-center gap-2 text-xs"><span className="h-2.5 w-2.5 shrink-0 rounded-full" style={{ backgroundColor: COLORS[index % COLORS.length] }} /><span className="min-w-0 flex-1 truncate text-slate-600" title={item.label}>{item.label}</span><span className="font-bold text-slate-900">{safeNumber(item.value)}</span></div>)}</div></div> : <Empty /> }
...
function LinkedReport({ title, description, href, navigate }) { return <Card padding="sm" className="h-full transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:shadow-brand-500/10 cursor-pointer" onClick={() => navigate(href)}><div className="flex h-full flex-col"><h3 className="font-bold text-slate-950">{title}</h3><p className="mt-1 flex-1 text-sm leading-5 text-slate-600">{description}</p><Button className="mt-4 w-full group transition-all" variant="outline">Open workspace</Button></div></Card> }
...
function ChartTooltip({ active, payload, label }) {
  ...
  return <div className="max-w-64 rounded-xl border border-white/40 bg-white/80 backdrop-blur-md px-4 py-3 text-xs shadow-xl shadow-slate-900/5 ring-1 ring-slate-900/5"><p className="mb-2 break-words font-black text-slate-900">{heading}</p>{payload.map((entry) => <div key={entry.dataKey || entry.name} className="flex items-center justify-between gap-4 py-1"><span className="flex items-center gap-2 text-slate-600 font-medium"><span className="h-2 w-2 rounded-full" style={{ backgroundColor: entry.color }}/>{entry.name || pretty(entry.dataKey)}</span><span className="font-black text-slate-950">{safeNumber(entry.value)}</span></div>)}</div>
}
