...
export default function JobMapDetailsPanel({ job, onClose, onApply, onSave, onTraining, onJobFair, onApplicationStatus, isApplying, isSaving, isRsvping }) {
  ...
  const matchPercentage = Math.round(renderedJob.match_percentage || 0)
    ...
    >
      ...
      <header className="flex shrink-0 items-start justify-between border-b border-slate-100 bg-white px-5 py-4">
        <div className="min-w-0 pr-3">
          <div className="mb-2 flex flex-wrap items-center gap-1.5">
            <span className={`rounded-full px-2.5 py-1 text-[10px] font-black ring-1 ring-inset ${matchTone(matchPercentage)}`}>{matchPercentage}% match</span>
      ...
      <div ref={contentRef} className="flex-1 overflow-y-auto overscroll-contain px-5 py-5">
        ...
        <div className="mt-5 space-y-2.5">
          ...
          {renderedJob.job_fair?.is_available_at_job_fair && <div className="rounded-xl border border-blue-100 bg-blue-50 p-3"><p className="flex items-center gap-2 text-xs font-black text-blue-950"><CalendarDays className="h-4 w-4" />Available at {renderedJob.job_fair.title}</p><p className="mt-1 text-[10px] leading-4 text-blue-700">{renderedJob.job_fair.date || 'Date to be announced'} · {renderedJob.job_fair.venue || 'Venue to be announced'}</p><button type="button" disabled={isRsvping} onClick={() => onJobFair(renderedJob)} className="mt-2 rounded-lg bg-blue-950 px-3 py-2 text-[10px] font-bold text-white disabled:opacity-50">{renderedJob.job_fair.has_rsvp ? 'View QR pass' : isRsvping ? 'Saving RSVP…' : 'RSVP to job fair'}</button></div>}
