...
class JobFairEcosystemFlowTest extends TestCase
{
    ...
    public function test_omnichannel_job_fair_flow_with_seeker_qr_registration(): void
    {
        ...
        Sanctum::actingAs($admin);
        $this->patchJson("/api/admin/job-fair-results/{$selfReportId}/review", ['status' => 'approved'])->assertOk();
    ...
    public function test_confirmation_slip_vacancy_list_derives_the_total_and_links_an_existing_posting(): void
    {
        [, $employer, $fairId] = $this->setUpFairWithEmployer('vacancylist');
        $vacancyId = DB::table('job_vacancies')->insertGetId([
            'employer_id' => $employer->employer_id, 'vacancies_count' => 2, 'status' => 'active',
            'created_at' => now(), 'updated_at' => now(),
        ], 'post_id');
        ...
        // number_of_job_vacancies is derived (2 + 3), never trusted from the client directly.
        $this->assertSame(5, $response->json('confirmation_slip.number_of_job_vacancies'));
        $this->assertCount(2, $response->json('confirmation_slip.vacancies'));
        $linkedRow = collect($response->json('confirmation_slip.vacancies'))->firstWhere('job_vacancy_id', $vacancyId);
        $this->assertSame('Machine Operator', $linkedRow['position_title']);
        $manualRow = collect($response->json('confirmation_slip.vacancies'))->firstWhere('job_vacancy_id', null);
        $this->assertSame('Packer', $manualRow['position_title']);
        ...
        $this->assertCount(1, $second->json('confirmation_slip.vacancies'));
    }

    public function test_admin_proxy_confirmation_slip_accepts_a_manual_vacancy_list(): void
    {
        [$admin, , $fairId] = $this->setUpFairWithEmployer('proxyvacancylist');
        ...
        $this->assertSame(4, $response->json('confirmation_slip.number_of_job_vacancies'));
        $this->assertSame('Warehouse Associate', $response->json('confirmation_slip.vacancies.0.position_title'));
        $this->assertNull($response->json('confirmation_slip.vacancies.0.job_vacancy_id'));
    }
    ...
    private function createTables(): void
    {
        ...
        Schema::create('seeker_occupations', function (Blueprint $t) { $t->id(); $t->unsignedBigInteger('seeker_id'); $t->string('occupation_title', 255); $t->unsignedTinyInteger('preference_order'); $t->timestamps(); $t->unique(['seeker_id', 'preference_order']); });
        Schema::create('seeker_languages', function (Blueprint $t) { $t->id(); $t->unsignedBigInteger('seeker_id'); $t->string('language'); $t->string('language_other', 100)->nullable(); $t->boolean('can_read')->default(false); $t->boolean('can_write')->default(false); $t->boolean('can_speak')->default(false); $t->boolean('can_understand')->default(false); $t->timestamps(); $t->unique(['seeker_id', 'language']); });
        Schema::create('seeker_work_locations', function (Blueprint $t) { $t->id(); $t->unsignedBigInteger('seeker_id'); $t->string('location_type'); $t->string('location_name', 255); $t->string('location_code', 10)->nullable(); $t->timestamps(); $t->unique(['seeker_id', 'location_name']); });
        ...
        Schema::create('seeker_work_experiences', function (Blueprint $t) { $t->id(); $t->unsignedBigInteger('seeker_id'); $t->string('company_name', 255); $t->string('company_address', 500)->nullable(); $t->string('position', 255); $t->unsignedInteger('number_of_months')->nullable(); $t->enum('employment_status', ['permanent', 'contractual', 'part_time', 'probationary', 'temporary', 'seasonal'])->nullable(); $t->date('start_date')->nullable(); $t->date('end_date')->nullable(); $t->timestamps(); });
