...
class EmployerJobFairController extends Controller
{
    ...
    public function interest(Request $request, JobFair $jobFair, JobFairService $service): JsonResponse
    {
        ...
        $participation = JobFairEmployer::updateOrCreate(
            ...
            ['participation_status' => 'requirements_pending', 'source' => 'employer_self_service', 'joined_at' => now(), 'responded_at' => now()],
        );
        ...
        return response()->json(['message' => 'Successfully joined the Job Fair.', 'participation' => $service->participationPayload($participation->fresh(['requirementSubmissions.requirement']))]);
    }
    ...
    public function respond(Request $request, JobFair $jobFair, JobFairService $service): JsonResponse
    {
        ...
        $participation->update([
            'participation_status' => $validated['response'] === 'accepted' ? 'requirements_pending' : $validated['response'],
            'responded_at' => now(),
    ...
    public function uploadRequirement(Request $request, JobFair $jobFair, JobFairRequirement $requirement, JobFairService $service): JsonResponse
    {
        ...
        $participation = $this->participation($jobFair, $employer);
        // Was checking $participation->status, a property JobFairEmployer
        // doesn't have (the column is participation_status) — always null,
        // so this guard silently never fired and employers could keep
        // replacing requirement documents after PESO had already approved
        // the whole participation.
        abort_if(in_array($participation->participation_status, ['approved', 'attended', 'encoded_results', 'report_generated']), 403, 'Participation is already approved. Cannot modify requirements.');
    ...
    public function confirmation(Request $request, JobFair $jobFair, JobFairService $service): JsonResponse
    {
        $employer = $this->employer($request);
        $participation = $this->participation($jobFair, $employer);
        abort_if(in_array($participation->participation_status, ['approved', 'attended', 'encoded_results', 'report_generated']), 403, 'Participation is already approved. Cannot modify confirmation slip.');
        $validated = $request->validate([
